...
include { TAXPASTA_MERGE as KRAKEN2_MERGE                                        } from '../../modules/nf-core/taxpasta/merge/main'
include { TAXPASTA_MERGE as BRACKEN_MERGE                                        } from '../../modules/nf-core/taxpasta/merge/main'
include { TAXPASTA_STANDARDISE as CENTRIFUGER_STANDARDISE                        } from '../../modules/nf-core/taxpasta/standardise/main'
include { TAXPASTA_STANDARDISE as KRAKEN2_STANDARDISE                            } from '../../modules/nf-core/taxpasta/standardise/main'
include { TAXPASTA_STANDARDISE as BRACKEN_STANDARDISE                            } from '../../modules/nf-core/taxpasta/standardise/main'
...
workflow TAXONOMIC_STANDARDISATION {
    ...
    CENTRIFUGER_MERGE(
        ch_input_for_taxpasta_merge.centrifuge.filter { meta, report ->
            report.size() > 1
        },
        'tsv',
        ch_taxpasta_tax_dir
    ...
    KRAKEN2_MERGE(
        ch_input_for_taxpasta_merge.kraken2.filter { meta, file ->
            file.size() > 1
        },
        'tsv',
        k2_taxonomy_db
    )
    ch_versions = ch_versions.mix(KRAKEN2_MERGE.out.versions)

    // Bracken - NCBI taxonomy
    BRACKEN_MERGE(
        ch_input_for_taxpasta_merge.bracken.filter { meta, file ->
            file.size() > 1
        },
        'tsv',
        k2_taxonomy_db
    )
    ch_versions = ch_versions.mix(BRACKEN_MERGE.out.versions)

    ch_merge_reports = ch_merge_reports
        .mix(CENTRIFUGER_MERGE.out.merged_profiles)
        .mix(KRAKEN2_MERGE.out.merged_profiles)
        .mix(BRACKEN_MERGE.out.merged_profiles)

    CENTRIFUGER_STANDARDISE(
        ch_input_for_taxpasta_stand.centrifuge,
        'tsv',
        ch_taxpasta_tax_dir
    )
    ...
    KRAKEN2_STANDARDISE(
        ch_input_for_taxpasta_stand.kraken2,
        'tsv',
        k2_taxonomy_db
    )
    ch_versions = ch_versions.mix(KRAKEN2_STANDARDISE.out.versions)

    BRACKEN_STANDARDISE(
        ch_input_for_taxpasta_stand.bracken,
        'tsv',
        k2_taxonomy_db
    )
    ch_versions = ch_versions.mix(BRACKEN_STANDARDISE.out.versions)
    ...
    PLOT_KRAKEN2(
        KRAKEN2_STANDARDISE.out.standardised_profile,
        "Kraken2, Taxpasta",
        k2_taxonomy_db,
        file("/mnt/workflow/definition/mag-v3.4.2/docs/images/mi_logo.png"),
        file(params.tax_prof_template, checkIfExists: true)
    ...
    // Bracken standardise
    PLOT_KRAKEN2BRACKEN(
        BRACKEN_STANDARDISE.out.standardised_profile,
        "Bracken, Taxpasta",
        k2_taxonomy_db,
        file("/mnt/workflow/definition/mag-v3.4.2/docs/images/mi_logo.png"),
        file(params.tax_prof_template, checkIfExists: true)
