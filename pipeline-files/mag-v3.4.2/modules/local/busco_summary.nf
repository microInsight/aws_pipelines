process BUSCO_SUMMARY {
    ...
    if ("${reference}" == false && failed_bins.sort().size() > 0)
        f = "-f ${failed_bins}"
    """
    chmod +x /mnt/workflow/definition/mag-v3.4.2/bin/summary_busco.py

    python3 /mnt/workflow/definition/mag-v3.4.2/bin/summary_busco.py $auto $ss $sd $f -o busco_summary.tsv
