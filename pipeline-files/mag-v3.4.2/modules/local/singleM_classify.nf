process SINGLEM_CLASSIFY {
    ...
    output:
    tuple val(meta), path("${meta.id}-${input_type}_profile.tsv"), path("${meta.id}-${input_type}_otu_table.csv"), emit: singleM_output
    ...
    if [ ${input_type} == "fasta" ]; then
        singlem pipe \\
            ...
            -p ${meta.id}-${input_type}_profile.tsv \\
            --taxonomic-profile-krona ${meta.id}-${input_type}_profile_krona.html \\
            --otu-table ${meta.id}-${input_type}_otu_table.csv \\
            --threads ${task.cpus} \\
