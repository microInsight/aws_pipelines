...
plot_B_colors = ["#ff4a24","#19D3F3","#F58518","#e60019","#006fdf","#FECB52","#636EFA",
				 ...
				 "#3969ac","#44aa99","#54A24B","#72B7B2","#7f3c8d","#9D755D","#aa4499",
				 "#AB63FA","#B279A2","#b3de69","#B6E880","#bebada","#c44a86","#cc6677",
				 "#ddcc77","#de57ff","#e59800","#e68310","#e73f74","#EECA3B","#EF553B",
				 "#fb8072","#fccde5","#FF6692","#FF97FF"]
...
def process_sylph(sylph_infile, sylph_metadata):
	...
	syl_merge['Species1'] = syl_merge['tax'].str.replace(r'd__;p__;o__;f__;g__;s__$', '',
													  regex=True).str.replace(r';p__;o__;f__;g__;s__$', '',
									   regex=True).str.replace(r';c__;o__;f__;g__;s__$', '',
									regex=True).str.replace(r';o__;f__;g__;s__$', '',
								 regex=True).str.replace(r';f__;g__;s__$', '',
								 regex=True).str.replace(r';g__;s__$', '',
								 regex=True).str.replace(r';s__$', '',
	...
	syl_merge['rank']
	syl_merge['Species2'] = syl_merge['Species1'].str.replace('s__', '').str.replace('g__',
																				  '').str.replace('f__', '').str.replace('o__',
															 '').str.replace('c__', '').str.replace('p__', '').str.replace('d__', '')
...
#
def merge_ccvals(sample_data, taxp_fig_A_data, taxp_fig_B_data, taxp_fig_A_data_centrifuger, taxp_fig_B_data_centrifuger,
				 fig_bracken_data_kraken2, fig_bracken_data_centrifuger, fig_syl_data):
	legend_count = 0
	context = {}
...
def parse_args():
	description = "'Plots individual and (optionally) merged input from read-based taxonomic classifiers. " \
    "Version: %s, Date: %s, Author: %s' % (__version__, __date__, __author__)"
...
def main():
	...
	context = merge_ccvals(sample_data, taxp_fig_A_data, taxp_fig_B_data, taxp_fig_A_data_centrifuger, taxp_fig_B_data_centrifuger,
						fig_bracken_data_kraken2, fig_bracken_data_centrifuger, fig_syl_data)
