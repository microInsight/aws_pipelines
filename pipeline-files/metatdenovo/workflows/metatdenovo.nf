...
workflow METATDENOVO {
    ...
    // relocate parameter calls and conditional statements from before workflow call to here so linters stop freaking out        
    ...
    if (params.hmmdir) {
            Channel.fromPath(params.hmmdir + params.hmmpattern, checkIfExists: true)
                .set { ch_hmmrs }
    } 
    else if (params.hmmfiles) {
            Channel.fromList(params.hmmfiles.tokenize(','))
                .map { [file(it)] }
                .set { ch_hmmrs }
    }
    ...
    if (params.user_assembly) {
        ...
        if (!params.user_assembly.endsWith('.gz')) {
            ...
            Channel.value([[id: assembly_name], file(params.user_assembly)])
    ...
    if (params.user_orfs_faa && params.user_orfs_gff) {
        Channel.value([[id: "${assembly_name}.${orfs_name}"], file(params.user_orfs_gff)])
            .set { ch_gff }
        Channel.value([[id: "${assembly_name}.${orfs_name}"], file(params.user_orfs_faa)])
            .set { ch_protein }
    }
